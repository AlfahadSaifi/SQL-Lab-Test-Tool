    ...
    <div class="row m-0">
        ...
        <div id="testReportSection" class="col-9 m-0 p-2 " style="width: 80%;">
            <div class="border border-gray shadow p-3 bg-white rounded">
                <h3 class="mb-2 text-center">Trainee LabTest Information</h3>
                <div class="row px-2">
                    <div class="col-md-3">
                        <div class="card mb-4">
                            <div class="card-body">
                    ...
                    <table id="myTable" class="table">
                        ...
                        <%! int count = 1; %>
                             <c:forEach var="labDetail" items="${TraineeLabReport}">
                                     ...
                                     <td>
                                         <c:choose>
                                             <c:when test="${labDetail.labTestResult == 'Pass'}">
                                                 <span class="badge bg-success">
                                                     <i style="color: green;"></i> Pass
                                                 </span>
                                             </c:when>
                                             <c:when test="${labDetail.labTestResult == 'Fail'}">
                                                 <span class="badge bg-danger">
                                                     <i  style="color: red;"></i> Fail
                                                 </span>
                                             </c:when>
                                             <c:otherwise>${labDetail.labTestResult}</c:otherwise>
                                         </c:choose>
                                     </td>
                                           <td>${labDetail.labTestReports.labTestInfo.labTestStatus}</td>
                                            <td><a class="py-2 px-3 rounded border bg-blue text-white text-sm " href="${pageContext.request.contextPath}/admin/viewDetailedReports?traineeId=${traineeId}&labTestId=${labDetail.labTestReports.labTestInfo.labTestId}&batchId=${batchId}&labTestName=${labDetail.labTestName}">View Detailed LabTestReport</a></td>
